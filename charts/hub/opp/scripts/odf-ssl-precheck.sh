    ...
    spec:
      ...
      - name: odf-ssl-extractor
        ...
        - |
          ...
          echo "7b. Updating ramen-hub-operator-config in openshift-gitops namespace..."
          ...
          if oc get configmap ramen-hub-operator-config -n openshift-gitops &>/dev/null; then
            echo "  ConfigMap exists, patching with base64-encoded CA bundle..."
            oc patch configmap ramen-hub-operator-config -n openshift-gitops \
              --type=merge \
            ...
            oc create configmap ramen-hub-operator-config -n openshift-gitops \
